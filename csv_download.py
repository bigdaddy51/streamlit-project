...
def run_csv_check():
    ...
    try:
        ...
        csv_file = "/tmp/student_funds.csv"
...
def download_csv():
    """
    Run the CSV check function and return the main CSV file as a downloadable attachment.
    """
    if run_csv_check():
        try:
...
@csv_download_bp.route('/download_duplicate_csv', methods=['GET'])
def download_duplicate_csv():
    """
    For demonstration, generate a duplicate CSV file (this can be adjusted for your logic)
    and return it as a download.
    """
    # Here you would include logic to identify duplicates.
    # For brevity, we create an empty duplicate CSV file with just the header.
    duplicate_csv = "/tmp/duplicate_student_funds.csv"
    with open(duplicate_csv, mode='w', newline='') as dup_file:
        ...
        writer.writerow(header)
    try:
        return send_file(duplicate_csv,
                         mimetype="text/csv",
                         attachment_filename="duplicate_student_funds.csv",
                         as_attachment=True)
    except Exception as e:
        logging.error(f"Error sending duplicate file: {e}")
        abort(500)